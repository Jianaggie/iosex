...
//  修改记录 ：把 self = [super init]－》self == [super init]
//
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
    ...
    CFDataRef nativeProp;
   
    if(theReadStream==NULL){
        // NSLog(@"--------------------theReadStream%@",theReadStream);
        return NO;
    }
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
    //__NSCFInputStream: 0x1ab5b0>
    //<__NSCFInputStream: 0x184ce0>
    //NSLog(@"--------------CFStreamEventType------%@",stream);
     if(theReadStream!=nil){
