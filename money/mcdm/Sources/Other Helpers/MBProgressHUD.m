...
- (void)layoutSubviews {
    ...
    if (detailsLabel.text)
    {
        ...
        if (detailsLabelSize.height > 0.f && (indicatorF.size.height > 0.f || label.text)) {
            totalSize.height += kPadding;
        }
    }
	...
	if (label.text && indicatorF.size.height > 0.f) {
		yPos += kPadding;
	}
	...
	if (detailsLabel.text && (indicatorF.size.height > 0.f || label.text)) {
		yPos += kPadding;
	}
